        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '\n*humsafar143  Working as Private!😎*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*humsafar143 is working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using WhatsAsena 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [WHATSASENA] --*' + 
                                    ...
                                    '\n_Hatayı https://chat.whatsapp.com/KE7xElFzU2F03IdiapFPuZ bu gruba iletebilirsiniz._\n\n' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [humsafar143] --*' + 
                                    '\n*humsafar143 an error has occurred!*'+
                                    ...
                                    '\n*Aslo you can join our support group:* https://chat.whatsapp.com/KE7xElFzU2F03IdiapFPuZ' +
                                    '\n*This message should have gone to your number (saved messages).*\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [humsafar143] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
                                        '\n_You can write to our support group for more help /n https://chat.whatsapp.com/KE7xElFzU2F03IdiapFPuZ._'
                                        , MessageType.text
                                    );
                                }    
                            }                      
